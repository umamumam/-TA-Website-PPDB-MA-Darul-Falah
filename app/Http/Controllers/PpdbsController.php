...
class PpdbsController extends Controller
{
    ...
    {
        $ppdbs= Ppdb::all();
        return view('ppdbs.index', ['ppdbs'=>$ppdbs]);
    }
    ...
    public function store(PpdbRequest $request)
    {
        ...
        // Cek apakah NIK yang dimasukkan sudah ada di dalam database
    $existingRecord = Ppdb::where('nik_peserta', $request->input('nik_peserta'))->first();

    // Jika NIK sudah ada, tolak pendaftaran
    if ($existingRecord) {
        return redirect()->back()->with('error', 'Anda sudah terdaftar. Tidak Dapat Melakukan Input Pendaftaran Lagi. Jika ada pertanyaan Hubungi Contact Person');
    }
        ...
        return view('ppdbs.show',['ppdb'=>$ppdb]);
    ...
    {
        $ppdb = Ppdb::findOrFail($id);
        return view('ppdbs.edit',['ppdb'=>$ppdb]);
    }
